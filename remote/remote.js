...
(function () {
  ...
  // Randomizer lock icons -- same Google Material Symbols "lock"/
  // "lock_open" glyphs the desktop panel uses (see LOCK_ICON in
  // fractalize-core.js), duplicated here rather than shared since this
  // page has no access to that file's own closure. Fill color is baked
  // into each variant (light on the default circle, dark once the
  // button goes solid white when locked), matching the desktop's own
  // two pre-built strings.
  const LOCK_ICON = {
    open:
  ...
  function buildControls(controls) {
    ...
    controls.forEach((c, i) => {
      ...
      if (c.kind === "range") {
        ...
        // Lockable sliders (see c.lockable, set on the desktop side) get
        // a round lock button to their left, matching the desktop
        // panel's own layout -- everything else keeps its full-width
        // slider with no wrapper, unchanged.
        let lockBtn;
        if (c.lockable) {
          const sliderRow = el("div", "remote-slider-row");
          lockBtn = el("button", "remote-lock");
          lockBtn.type = "button";
          lockBtn.setAttribute("aria-label", "Lock " + c.label + " from the Randomizer");
          lockBtn.addEventListener("click", () => send({ t: "lock", key: c.key }));
          sliderRow.appendChild(lockBtn);
          sliderRow.appendChild(input);
          row.appendChild(sliderRow);
        } else {
          row.appendChild(input);
        }
        rangeEls[c.key] = { input, val, lockBtn };
      } else {
  ...
  function applyControlValues(controls) {
    controls.forEach((c) => {
      if (c.kind === "range") {
        ...
        if (r.lockBtn) {
          r.lockBtn.classList.toggle("is-active", !!c.locked);
          r.lockBtn.innerHTML = c.locked ? LOCK_ICON.closed : LOCK_ICON.open;
          r.lockBtn.setAttribute("aria-pressed", String(!!c.locked));
        }
